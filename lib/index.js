...
module.exports.handler = async function(event, context) {
  ...
  if (event.fragment) {
    try {
      ...
      for (const resourceName of Object.keys(templateFragment.Resources || {})) {
        ...
        const calloutResource = {
          ...
          Properties: {
            ...
            // create a copy of the properties since those will be modified in the depending method
            // we should do a deep clone here, but this library currently does not depend on any other
            // library and pulling in Lodash is a bit overkill just to setup a deep clone, which is
      ...
      console.log('AWS Architect Macro transform success', templateFragment);
      return {
      ...
      console.log('AWS Architect Macro transform failure error', JSON.stringify(error, null, 0), error);
      return {
        requestId: event.requestId,
        status: 'failure',
        fragment: {}
  ...
  await handler.handler(event.RequestType, properties, event.OldResourceProperties && event.OldResourceProperties.Properties);
  return {};
};
