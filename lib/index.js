  ...
  if (event.fragment) {
    try {
      ...
      for (const resourceName of Object.keys(templateFragment.Resources || {})) {
        ...
        if (!resourceName.startswith(awsArchitectCustomResourceNamePrefix)) {
          continue;
        }

        const functionName = resourceName.slice(awsArchitectCustomResourceNamePrefix.length);
        ...
        const calloutResource = {
          ...
          FunctionName: macroFunctionName,
          Version: '1.0',
          Properties: {
        ...
        handler.getResources(templateFragment.Resources, calloutResource, functionName);
      }
