...
module.exports.getResources = function(resources, calloutResource, resourceName) {
  ...
  const originalResource = resources[resourceName];

  // These already exist in verification list.
  const skipWildcardMaps = {};
  [originalResource.Properties.DomainName].concat(originalResource.Properties.SubjectAlternativeNames || []).filter(d => d[0] !== '*').forEach(n => {
    skipWildcardMaps[n] = true;
  });
  [originalResource.Properties.DomainName].concat(originalResource.Properties.SubjectAlternativeNames || []).filter(d => d[0] !== '*' || !skipWildcardMaps[d.replace(/^(\*\.)/, '')]).map((_, index) => {
    ...
    delete resources[subAltResourceName].Properties.CertificateName;
  });
