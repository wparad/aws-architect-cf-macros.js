...
module.exports.getResources = function(resources, calloutResource, resourceName) {
  ...
  clonedObject.Properties.SubjectAlternativeNames.filter(d => d[0] !== '*').forEach(n => {
    skipWildcardMaps[n] = true;
  });
  clonedObject.Properties.SubjectAlternativeNames.filter(d => d[0] !== '*' || !skipWildcardMaps[d.replace(/^(\*\.)/, '')]).map((_, index) => {
