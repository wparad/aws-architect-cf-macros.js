...
module.exports.handler = async function(mode, properties) {
  ...
  if (!certificateName) {
    await cloudFormationExecutionContext.completeRequest(cloudFormationExecutionContext.statuses.FAILED, { title: 'CertificateName is required', mode, properties });
    return;
  }
  ...
  try {
    ...
    await cloudFormationExecutionContext.completeRequest(cloudFormationExecutionContext.statuses.FAILED, { title: `Certificate with CertificateName ${certificateName} not found.`, certificatesResult, mode, properties });
  } catch (error) {
    await cloudFormationExecutionContext.completeRequest(cloudFormationExecutionContext.statuses.FAILED, { title: 'Failed to get Certificate data', error, mode, properties });
  }
};
